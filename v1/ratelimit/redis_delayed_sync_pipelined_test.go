...
func TestRedisDelayedSyncPipelined(t *testing.T) {
	...
	t.Run("Synchronization tests", func(t *testing.T) {
		t.Run("diffs should be equal after syncAll if there are no actions in between", func(t *testing.T) {
			randomString := test_utils.RandString(10)
		...
		t.Run("RedisDelayedSyncCorruptedRemotePolicyUploadLocal", func(t *testing.T) {
			...
			testCases := []struct {
				...
				{
					name: "ForceN on both rate limiters followed by corrupt",
					scenario: func() {
				...
				{
					name: "ForceN on alpha, syncAll on alpha, corrupt, ForceN on beta, syncAll on beta",
					scenario: func() {
						_, _ = ratelimiterAlpha.ForceN(randomString, 3, 1, 1000)
						_ = ratelimiterAlpha.syncAll()
						corrupt()
				...
				{
					name: "ForceN in alpha, syncAll on alpha, corrupt, ForceN on both rate limiters, syncAll",
					scenario: func() {
						_, _ = ratelimiterAlpha.ForceN(randomString, 3, 1, 1000)
						_ = ratelimiterAlpha.syncAll()
						corrupt()
				...
				{
					name: "ForceN on alpha, syncAll on alpha, corrupt, ForceN on both, syncAll on both",
					// This test case shows that if a server manages to addSyncCommand twice before the other server even syncs once after corruption
					// We lost delta of the addSyncCommand that happened before the corruption
					scenario: func() {
						_, _ = ratelimiterAlpha.ForceN(randomString, 3, 1, 1000)
						_ = ratelimiterAlpha.syncAll()
						corrupt()
	...
	t.Run("keyExpiry", func(t *testing.T) {
		...
		randomString := test_utils.RandString(10)
		_, err = ratelimiterAlpha.ForceN(randomString, 3, 1, 1)
		require.NoError(t, err)
...
func Test_GetResetAt_ShouldReturnNonZeroAfterUse_Pipelined(t *testing.T) {
...
func Test_GetResetAt_DefaultIsZero_Pipelined(t *testing.T) {
	rdb := redis.NewClient(&redis.Options{Addr: "localhost:6379", DB: 9})
	defer rdb.FlushDB(context.Background())
